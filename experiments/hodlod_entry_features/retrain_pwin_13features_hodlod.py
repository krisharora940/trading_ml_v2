...
OUT_MODEL    = os.path.join(BASE, "entry_model_pwin_13features_retrained_hodlod_entry.joblib")
OUT_CSV      = os.path.join(BASE, "ml_features_13feat_retrain_hodlod_entry.csv")

BASE_FEATURES_CSV = os.path.join(BASE, "ml_features_13features.csv")
...
for _, tr in bt_u.iterrows():
    ...
    # New: HOD relative to entry (long) / LOD relative to entry (short), normalized by ATR
    if direction == 'long':
        hod_rel_entry_atr = (day_high - entry_price_est) / atr_at_entry if atr_at_entry > 0 else 0.0
        lod_rel_entry_atr = 0.0
    else:
        hod_rel_entry_atr = 0.0
        lod_rel_entry_atr = (entry_price_est - day_low) / atr_at_entry if atr_at_entry > 0 else 0.0
...
print("\nMerging with base 13-feature dataset …")
base_df = pd.read_csv(BASE_FEATURES_CSV)
before_merge = len(feat_df)
merged = pd.concat([base_df, feat_df], ignore_index=True)
merged_before_dedup = len(merged)
merged = merged.drop_duplicates(subset=['open_date','direction','entry_time','pivot_time'])
print(f"  base rows: {len(base_df)}")
print(f"  new rows:  {before_merge}")
print(f"  merged rows (pre-dedup): {merged_before_dedup}")
print(f"  merged rows (deduped):   {len(merged)}")

merged.to_csv(OUT_CSV, index=False)
print(f"  Saved merged feature matrix → {OUT_CSV}")

# ── Train ────────────────────────────────────────────────────────────────────
print("\nTraining pwin model …")
X = merged[ML_FEATURES].fillna(0.0).values
y = merged['label_win'].astype(int).values
