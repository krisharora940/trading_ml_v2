        ...
        for _, ev in events.iterrows():
            ...
            if in_trade and trades:
                ...
                if current.outcome is None:
                        ...
                        if direction == 'long':
                            hard_stop_price = float(current.entry_price - (HARD_MAX_LOSS_DOLLARS / (2.0 * float(current.contracts or 1))))
                            if low <= hard_stop_price:
                                hit_time = first_30s_target_hit(ev['timestamp'], 'short', hard_stop_price)
                                current.outcome = 'loss'
                                current.exit_time = hit_time if hit_time is not None else t
                                current.exit_price = hard_stop_price
                                current.pnl = float(current.exit_price - current.entry_price) * float(current.contracts or 0)
                                current.stop_time = current.exit_time
                                current.stop_price = hard_stop_price
                            ...
                            if high >= hard_stop_price:
                                hit_time = first_30s_target_hit(ev['timestamp'], 'long', hard_stop_price)
                                current.outcome = 'loss'
                                current.exit_time = hit_time if hit_time is not None else t
                                current.exit_price = hard_stop_price
                                current.pnl = float(current.entry_price - current.exit_price) * float(current.contracts or 0)
