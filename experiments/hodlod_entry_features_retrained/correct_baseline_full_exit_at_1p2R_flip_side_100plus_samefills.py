...
FLIP_SIDE_THRESHOLD_CONTRACTS = 100
            ...
            if ev['kind'] == '1m':
                ...
                if candidate_active and direction == 'long' and close < zone_low:
                    if in_trade and trades and trades[-1].outcome is None:
                        ...
                        current.pnl = float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0
                    ...
                    if in_trade and trades and trades[-1].outcome is None:
                        ...
                        current.stop_time = t
                        current.stop_price = float(stop_price) if stop_price is not None else None
                        current.pnl = float(current.entry_price - current.exit_price) * float(current.contracts or 0) * 2.0
                        current.exit_reason = 'direction_flip'
                        in_trade = False
                        daily_pnl_dollars += float(current.pnl) * 2.0
                        scale_out_active = False
                ...
                if reentry_seen and not entry_triggered:
                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            original_risk = risk
                            risk_dollars_per_contract = original_risk * 2.0
                            if original_risk < MIN_RISK_PTS:
                                entry_triggered = False
                                in_trade = False
                                stop_price = None
                                target_price = None
                            ...
                            if mirror_mode:
                                ...
                                # Preserve the original entry time/price/fills; only mirror the side and exit geometry.
                                if direction == 'long':
                                    # Original short -> mirrored long: profit on close through original stop, loss on wick to original target.
                                    stop_price = original_target_price
                                    target_price = original_stop_price
                                else:
                                    # Original long -> mirrored short: profit on close through original stop, loss on wick to original target.
                                    stop_price = original_target_price
                                    target_price = original_stop_price
                                risk = original_risk
                            else:
            ...
            if ev['kind'] == '1m' and in_trade and trades:
                ...
                if current.outcome is None:
                        ...
                        if direction == 'long':
                            ...
                            if current.mirror_mode:
                                # Mirrored long: loss on wick through original target, profit on close through original stop.
                                stop_hit = low <= active_stop_price
                                target_hit = close >= target_price
                            else:
                            ...
                            if (target_hit if current.mirror_mode else high >= target_price):
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
                                    ...
                                    leg_trade = Trade(
                                        ...
                                        pnl=float(leg_exit_price - current.entry_price) * float(leg_q) * 2.0,
                                        contracts=leg_q,
                                    ...
                                    current.pnl = float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0
                                    current.exit_reason = 'target_close' if current.mirror_mode else 'target'
                                    in_trade = False
                                    daily_pnl_dollars += float(current.pnl) * 2.0
                                    scale_out_active = False
                                    scale_out_stage = 0
                                ...
                                if be_wick_hit:
                                    ...
                                    if current.mirror_mode:
                                        current.exit_price = round_to_tick(float(active_stop_price - (SLIPPAGE_TICKS * TICK_SIZE)))
                                        current.exit_reason = 'hard_stop'
                                    else:
                                        current.exit_price = round_to_tick(float(close - (SLIPPAGE_TICKS * TICK_SIZE)))
                            ...
                            active_stop_price = current.stop_price if current.stop_price is not None else stop_price
                            if current.mirror_mode:
                                # Mirrored short: loss on wick through original target, profit on close through original stop.
                            ...
                            if (target_hit if current.mirror_mode else low <= target_price):
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
                                    ...
                                    leg_trade = Trade(
                                        ...
                                        exit_time=leg_exit_time,
                                        exit_price=leg_exit_price,
                                        pnl=float(current.entry_price - leg_exit_price) * float(leg_q) * 2.0,
                                        contracts=leg_q,
                                        risk_dollars=float(current.risk * 2.0 * leg_q),
                                        outcome='win',
                                        exit_reason=f'target_scale{scale_out_stage + 1}',
