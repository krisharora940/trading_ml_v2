                ...
                if reentry_seen and not entry_triggered:
                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            if contracts >= FLIP_SIDE_THRESHOLD_CONTRACTS:
                                direction = 'short' if original_direction == 'long' else 'long'
                                if direction == 'long':
                                    entry_price = round_to_tick(close + (SLIPPAGE_TICKS * TICK_SIZE))
                                    # Mirrored logic: profit is 1R close-through; loss is hard 1.2R wick.
                                    stop_distance = max(1.2 * original_risk, MIN_PROFIT_TARGET_PTS)
                                    stop_price = round_to_tick(entry_price - stop_distance)
                                    target_price = round_to_tick(entry_price + original_risk)
                                    risk = original_risk
                                else:
                                    entry_price = round_to_tick(close - (SLIPPAGE_TICKS * TICK_SIZE))
                                    stop_distance = max(1.2 * original_risk, MIN_PROFIT_TARGET_PTS)
            ...
            if ev['kind'] == '1m' and in_trade and trades:
                ...
                if current.outcome is None:
                        ...
                        if direction == 'long':
                            ...
                            mirrored_large = (current.contracts or 0) >= FLIP_SIDE_THRESHOLD_CONTRACTS
                            if mirrored_large:
                            ...
                            if stop_hit or be_wick_hit:
                                ...
                                reentry_time = None
                                last_3_strong = []
                            elif (target_hit if mirrored_large else high >= target_price):
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
                                    ...
                                    hit_time = first_30s_target_hit(ev['timestamp'], direction, target_price)
                                    mirrored_large = (current.contracts or 0) >= FLIP_SIDE_THRESHOLD_CONTRACTS
                                    current.exit_time = t if mirrored_large else (hit_time if hit_time is not None else t)
                                    current.exit_price = round_to_tick(float((close - (SLIPPAGE_TICKS * TICK_SIZE)) if mirrored_large else (target_price - (SLIPPAGE_TICKS * TICK_SIZE))))
                                    current.pnl = float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0
                                    current.exit_reason = 'target_close' if mirrored_large else 'target'
                                    in_trade = False
                                    daily_pnl_dollars += float(current.pnl) * 2.0
                                    scale_out_active = False
                                    scale_out_stage = 0
                            ...
                            if stop_hit or be_wick_hit:
                                ...
                                reentry_time = None
                                last_3_strong = []
                            elif (target_hit if mirrored_large else low <= target_price):
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
