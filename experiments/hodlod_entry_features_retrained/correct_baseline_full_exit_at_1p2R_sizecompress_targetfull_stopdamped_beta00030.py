...
SIZE_COMPRESS_ANCHOR = 20.0
SIZE_COMPRESS_BETA = 0.30
...
def compress_contracts(raw_contracts: int) -> int:
    """Smoothly compress large sizes while preserving trade ordering."""
    if raw_contracts <= SIZE_COMPRESS_ANCHOR:
        return int(raw_contracts)
    adjusted = int((SIZE_COMPRESS_ANCHOR * ((raw_contracts / SIZE_COMPRESS_ANCHOR) ** SIZE_COMPRESS_BETA)) // 1)
    return max(1, min(raw_contracts, adjusted))


def stop_widening_factor(raw_contracts: int, adjusted_contracts: int) -> float:
    if adjusted_contracts <= 0:
        return 1.0
    return (raw_contracts / adjusted_contracts) ** 0.5

def target_widening_factor(raw_contracts: int, adjusted_contracts: int) -> float:
    if adjusted_contracts <= 0:
        return 1.0
    return raw_contracts / adjusted_contracts
                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            if contracts < 1:
                                ...
                                target_price = None
                                continue
                            contracts = compress_contracts(contracts)
                            stop_risk = risk * stop_widening_factor(raw_contracts, contracts)
                            target_risk = risk * target_widening_factor(raw_contracts, contracts)
                            risk = stop_risk
                            if direction == 'long':
                                stop_price = round_to_tick(entry_price - stop_risk)
                                target_distance = max(1.2 * target_risk, MIN_PROFIT_TARGET_PTS)
                                target_price = round_to_tick(entry_price + target_distance)
                            else:
                                stop_price = round_to_tick(entry_price + stop_risk)
                                target_distance = max(1.2 * target_risk, MIN_PROFIT_TARGET_PTS)
