...
class Trade:
    ...
    realwin_min_limit_active: bool = False
    realwin_min_limit_price: Optional[float] = None
    realwin_min_limit_armed_time: Optional[pd.Timestamp] = None
...
MIN_ENTRY_CONTRACTS = 20
REAL_WIN_MIN_PTS = 2.5
...
def realized_pnl(virtual_pnl: float, mirror_mode: bool) -> float:
    return -virtual_pnl if mirror_mode else virtual_pnl
...
def run_engine(df_1m: pd.DataFrame, df_30s: pd.DataFrame, allow_counter_candle_entry: bool, pwin_thresh: float = PWIN_THRESH_DEFAULT) -> List[Trade]:
    ...
    for day, day_1m in df_1m.groupby('day'):
        ...
        for _, ev in events.iterrows():
            ...
            if ev['kind'] == '1m':
                ...
                if candidate_active and direction == 'long' and close < zone_low:
                    if in_trade and trades and trades[-1].outcome is None:
                        ...
                        current.pnl = realized_pnl(float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                    ...
                    if in_trade and trades and trades[-1].outcome is None:
                        ...
                        current.stop_time = t
                        current.stop_price = float(stop_price) if stop_price is not None else None
                        current.pnl = realized_pnl(float(current.entry_price - current.exit_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                        current.exit_reason = 'direction_flip'
                        in_trade = False
                        daily_pnl_dollars += float(current.pnl) * 2.0
                        scale_out_active = False
                ...
                if reentry_seen and not entry_triggered:
                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            if direction == 'long':
                                entry_price = round_to_tick(close + (SLIPPAGE_TICKS * TICK_SIZE))
                            else:
                                entry_price = round_to_tick(close - (SLIPPAGE_TICKS * TICK_SIZE))
                            if direction == 'long':
                                stop_price = round_to_tick(pivot)
                            ...
                            if contracts < MIN_ENTRY_CONTRACTS:
                                # Skip trades below the minimum entry-size filter.
                                entry_triggered = False
                                in_trade = False
                                stop_price = None
                                target_price = None
                            ...
                            target_distance = 1.2 * risk if mirror_mode else max(1.2 * risk, MIN_PROFIT_TARGET_PTS)
                            if direction == 'long':
                                target_price = round_to_tick(entry_price + target_distance)
                            else:
                                target_price = round_to_tick(entry_price - target_distance)

                            # Full exit experiment: all trades exit 100% at 1.2R.
                            scale_out_stage = 0
                            scale_out_plan = [(contracts, 1.2)]
                            scale_out_active = len(scale_out_plan) > 1 and all(q > 0 for q, _ in scale_out_plan)
                            trades.append(Trade(
                                day=str(day),
                                direction=mirrored_direction(direction, mirror_mode),
            ...
            # Manage trade (stop/target) using 1m bars only.
            if ev['kind'] == '1m' and in_trade and trades:
                current = trades[-1]
                if current.outcome is None:
                        ...
                        if direction == 'long':
                            if current.realwin_min_limit_active and current.realwin_min_limit_price is not None and low <= current.realwin_min_limit_price:
                                hit_time = first_30s_target_hit(ev['timestamp'], 'short', current.realwin_min_limit_price)
                                current.outcome = 'loss'
                                current.exit_time = hit_time if hit_time is not None else t
                                current.exit_price = float(current.realwin_min_limit_price)
                                current.pnl = realized_pnl(float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                                current.exit_reason = 'realwin_min_limit'
                                current.realwin_min_limit_active = False
                                in_trade = False
                                daily_pnl_dollars += float(current.pnl) * 2.0
                                scale_out_active = False
                                scale_out_stage = 0
                            ...
                            active_stop_price = current.stop_price if current.stop_price is not None else stop_price
                            stop_hit = close <= active_stop_price
                            be_wick_hit = (
                                current.stop_price is not None
                                and current.stop_price == current.entry_price
                                and low <= current.entry_price
                            )
                            if high >= target_price:
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
                                    ...
                                    leg_exit_price = round_to_tick(float(target_price - (SLIPPAGE_TICKS * TICK_SIZE)))
                                    leg_trade = Trade(
                                        day=current.day,
                                        direction=current.direction,
                                        virtual_direction=current.virtual_direction,
                                        entry_time=current.entry_time,
                                        entry_price=current.entry_price,
                                        exit_time=leg_exit_time,
                                        exit_price=leg_exit_price,
                                        pnl=realized_pnl(float(leg_exit_price - current.entry_price) * float(leg_q) * 2.0, current.mirror_mode),
                                        contracts=leg_q,
                                        ...
                                        mirror_mode=current.mirror_mode
                                    )
                                    trades[-1] = leg_trade
                                    daily_pnl_dollars += float(leg_trade.pnl) * 2.0
                                    scale_out_stage += 1
                                    target_price = current.entry_price + next_r * current.risk
                                    next_stop_price = current.stop_price
                                    if current.contracts and current.contracts > 35 and scale_out_stage >= 2:
                                        next_stop_price = float(current.entry_price)
                                    ...
                                    hit_time = first_30s_target_hit(ev['timestamp'], direction, target_price)
                                    current.exit_time = hit_time if hit_time is not None else t
                                    current.exit_price = round_to_tick(float(target_price - (SLIPPAGE_TICKS * TICK_SIZE)))
                                    current.pnl = realized_pnl(float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                                    ...
                                    reentry_time = None
                                    last_3_strong = []
                            elif stop_hit or be_wick_hit:
                                current.outcome = 'loss'
                                current.exit_time = t
                                if be_wick_hit:
                                    current.exit_price = round_to_tick(float(current.entry_price))
                                    current.exit_reason = 'break_even_stop'
                                else:
                                    current.exit_price = round_to_tick(float(close - (SLIPPAGE_TICKS * TICK_SIZE)))
                                    current.exit_reason = 'stop'
                                actual_win_points = float(current.entry_price - current.exit_price) if current.mirror_mode else 0.0
                                if current.mirror_mode and not be_wick_hit and actual_win_points > 0.0 and actual_win_points < REAL_WIN_MIN_PTS:
                                    current.outcome = None
                                    current.exit_time = None
                                    current.exit_price = None
                                    current.exit_reason = None
                                    current.realwin_min_limit_active = True
                                    current.realwin_min_limit_price = round_to_tick(float(current.entry_price - REAL_WIN_MIN_PTS))
                                    current.realwin_min_limit_armed_time = t
                                    continue
                                current.pnl = realized_pnl(float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                                current.stop_time = t
                                current.stop_price = float(active_stop_price)
                                in_trade = False
                                daily_pnl_dollars += float(current.pnl) * 2.0
                                scale_out_active = False
                                scale_out_stage = 0
                            ...
                            if current.realwin_min_limit_active and current.realwin_min_limit_price is not None and high >= current.realwin_min_limit_price:
                                hit_time = first_30s_target_hit(ev['timestamp'], 'long', current.realwin_min_limit_price)
                                current.outcome = 'loss'
                                current.exit_time = hit_time if hit_time is not None else t
                                current.exit_price = float(current.realwin_min_limit_price)
                                current.pnl = realized_pnl(float(current.entry_price - current.exit_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                            ...
                            if low <= target_price:
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
                                    ...
                                    leg_trade = Trade(
                                        ...
                                        exit_time=leg_exit_time,
                                        exit_price=leg_exit_price,
                                        pnl=realized_pnl(float(current.entry_price - leg_exit_price) * float(leg_q) * 2.0, current.mirror_mode),
                                        contracts=leg_q,
                                        risk_dollars=float(current.risk * 2.0 * leg_q),
                                        outcome='win',
                                        exit_reason=f'target_scale{scale_out_stage + 1}',
                                ...
                                if be_wick_hit:
                                    ...
                                    current.exit_price = round_to_tick(float(close + (SLIPPAGE_TICKS * TICK_SIZE)))
                                    current.exit_reason = 'stop'
                                actual_win_points = float(current.exit_price - current.entry_price) if current.mirror_mode else 0.0
                                if current.mirror_mode and not be_wick_hit and actual_win_points > 0.0 and actual_win_points < REAL_WIN_MIN_PTS:
                                    current.outcome = None
                                    current.exit_time = None
                                    current.exit_price = None
        ...
        if trades and trades[-1].outcome is None:
            ...
            current.exit_time = t
            if current.virtual_direction == 'long':
                current.exit_price = round_to_tick(close - (SLIPPAGE_TICKS * TICK_SIZE))
            else:
                current.exit_price = round_to_tick(close + (SLIPPAGE_TICKS * TICK_SIZE))
            if current.virtual_direction == 'long':
                current.pnl = realized_pnl(float(current.exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
