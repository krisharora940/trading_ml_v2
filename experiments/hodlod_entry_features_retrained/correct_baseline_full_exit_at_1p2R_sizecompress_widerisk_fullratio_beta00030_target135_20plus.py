...
def widening_factor(raw_contracts: int, adjusted_contracts: int) -> float:
    if adjusted_contracts <= 0:
        return 1.0
    return raw_contracts / adjusted_contracts


def target_r_multiple(raw_contracts: int) -> float:
    return LARGE_TRADE_TARGET_R if raw_contracts >= 20 else 1.2
                ...
                if reentry_seen and not entry_triggered:
                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            widen = widening_factor(raw_contracts, contracts)
                            risk = risk * widen
                            if direction == 'long':
                                stop_price = round_to_tick(entry_price - risk)
                                target_distance = max(target_r_multiple(raw_contracts) * risk, MIN_PROFIT_TARGET_PTS)
                                target_price = round_to_tick(entry_price + target_distance)
                            else:
                                stop_price = round_to_tick(entry_price + risk)
                                target_distance = max(target_r_multiple(raw_contracts) * risk, MIN_PROFIT_TARGET_PTS)
                            ...
                            scale_out_stage = 0
                            scale_out_plan = [(contracts, target_r_multiple(raw_contracts))]
