                ...
                if reentry_seen and not entry_triggered:
                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            # Scale-out plan:
                            #   contracts > 35 => 25% at 4R, 25% at 6.5R, 25% at 10R, 25% at 20R
                            #                    after 6.5R, remaining 50% stop moves to break-even on wick
                            ...
                            if contracts > 35:
                                q = contracts // 4
                                rem = contracts - 3 * q
                                scale_out_plan = [(q, 4.0), (q, 6.5), (q, 10.0), (rem, 20.0)]
                            elif contracts >= 13:
                                q = contracts // 4
                                rem = contracts - 3 * q
            ...
            if in_trade and trades:
                ...
                if current.outcome is None:
                        ...
                        if direction == 'long':
                            ...
                            be_wick_hit = (
                                current.contracts is not None
                                and current.contracts > 0
                                and current.stop_price is not None
                            ...
                            if stop_hit or be_wick_hit:
                                ...
                                current.exit_time = t
                                if be_wick_hit:
                                    current.exit_price = float(current.entry_price)
