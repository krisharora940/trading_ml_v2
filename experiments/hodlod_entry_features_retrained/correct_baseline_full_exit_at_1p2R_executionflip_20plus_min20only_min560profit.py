...
def can_take_non_timeout_exit(actual_pnl_dollars: float) -> bool:
    return actual_pnl_dollars <= 0 or actual_pnl_dollars >= MIN_PROFIT_EXIT_DOLLARS
            ...
            if ev['kind'] == '1m':
                ...
                if candidate_active and direction == 'long' and close < zone_low:
                    ...
                    if in_trade and trades and trades[-1].outcome is None:
                        ...
                        candidate_pnl = realized_pnl(float(exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                        if can_take_non_timeout_exit(float(candidate_pnl)):
                            current.outcome = 'loss'
                            current.exit_time = t
                            current.exit_price = exit_price
                            current.stop_time = t
                            current.stop_price = float(stop_price) if stop_price is not None else None
                            current.pnl = candidate_pnl
                    ...
                    if in_trade and trades and trades[-1].outcome is None:
                        current = trades[-1]
                        exit_price = float(close)
                        candidate_pnl = realized_pnl(float(current.entry_price - exit_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                        if can_take_non_timeout_exit(float(candidate_pnl)):
                            current.outcome = 'loss'
                            current.exit_time = t
                            current.exit_price = exit_price
                        ...
                        if direction == 'long':
                            ...
                            if high >= target_price:
                                if scale_out_active and scale_out_stage < len(scale_out_plan) - 1:
                                    ...
                                    hit_time = first_30s_target_hit(ev['timestamp'], direction, target_price)
                                    exit_time = hit_time if hit_time is not None else t
                                    exit_price = round_to_tick(float(target_price - (SLIPPAGE_TICKS * TICK_SIZE)))
                                    candidate_pnl = realized_pnl(float(exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                                    if can_take_non_timeout_exit(float(candidate_pnl)):
                                        current.outcome = 'win'
                                ...
                                if be_wick_hit:
                                    exit_price = round_to_tick(float(current.entry_price))
                                    exit_reason = 'break_even_stop'
                                else:
                                    exit_price = round_to_tick(float(close - (SLIPPAGE_TICKS * TICK_SIZE)))
                                    exit_reason = 'stop'
                                candidate_pnl = realized_pnl(float(exit_price - current.entry_price) * float(current.contracts or 0) * 2.0, current.mirror_mode)
                                if can_take_non_timeout_exit(float(candidate_pnl)):
                                    current.outcome = 'loss'
