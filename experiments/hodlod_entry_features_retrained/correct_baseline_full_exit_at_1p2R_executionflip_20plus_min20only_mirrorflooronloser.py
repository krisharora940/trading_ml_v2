                    ...
                    if retrace is not None:
                        ...
                        if (ml_model is not None and ml_allows_entry) or (ml_model is None and (strong_count >= 2 or very_strong)):
                            ...
                            if direction == 'long':
                                ...
                                entry_price = round_to_tick(close - (SLIPPAGE_TICKS * TICK_SIZE))
                            raw_stop_price = round_to_tick(pivot)
                            if direction == 'long':
                                raw_risk = entry_price - raw_stop_price
                            else:
                                raw_risk = raw_stop_price - entry_price

                            # Position sizing and flip threshold are chosen off the raw setup first.
                            risk_dollars_per_contract = raw_risk * 2.0
                            if raw_risk < MIN_RISK_PTS:
                                entry_triggered = False
                                in_trade = False
                                stop_price = None
                                target_price = None
                                continue
                            raw_contracts = int(500 // risk_dollars_per_contract) if risk_dollars_per_contract > 0 else 0
                            if raw_contracts > MAX_CONTRACTS:
                                raw_contracts = MAX_CONTRACTS
                            mirror_mode = raw_contracts >= FLIP_SIDE_THRESHOLD_CONTRACTS

                            # For mirrored trades, move the 2.5-point floor onto the virtual loser side.
                            # That means the virtual stop distance gets the floor, while the virtual target
                            # goes back to pure 1.2R off the effective risk.
                            risk = max(raw_risk, MIN_PROFIT_TARGET_PTS) if mirror_mode else raw_risk
                            if direction == 'long':
                                stop_price = round_to_tick(entry_price - risk)
                                target_price = round_to_tick(entry_price + (1.2 * risk if mirror_mode else max(1.2 * risk, MIN_PROFIT_TARGET_PTS)))
                            else:
                                stop_price = round_to_tick(entry_price + risk)
                                target_price = round_to_tick(entry_price - (1.2 * risk if mirror_mode else max(1.2 * risk, MIN_PROFIT_TARGET_PTS)))
