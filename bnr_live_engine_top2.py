...
# top-2 mode: no fixed threshold — entry allowed when current score >= min of top-2 scores seen this setup
...
class BNRLiveEngine:
    ...
    def __init__(self, out_dir: str = "output/bnr_live",
        ...
        # Load ML model (top-2 bundle: dict with 'model' and 'features' keys)
        if os.path.exists(MODEL_PATH):
            _bundle = joblib.load(MODEL_PATH)
            self._model    = _bundle['model']
            self._features = _bundle['features']
            print(f"[BNR-top2] ML model loaded ({len(self._features)} features, top-2 scoring mode)")
        else:
            self._model = None
            self._features = None
            print("[BNR-top2] ML model not found — strong-candle fallback active")
            ...
            if self._model is not None and self._last_30s_close is not None:
                if self._flem is not None and self._pivot is not None and self._flem != self._pivot:
                    ...
                    if score is not None:
                        self._top2_scores.append(score)
                        self._top2_scores = sorted(self._top2_scores, reverse=True)[:2]
                        ml_allows_entry = (score >= min(self._top2_scores)
                                           if len(self._top2_scores) == 2 else False)
