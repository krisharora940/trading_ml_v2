#!/usr/bin/env python3
"""
BNR Live Trading Engine — 13-feature pwin model variant

Identical to bnr_live_engine_pwin.py except:
  - Uses entry_model_pwin_13features_retrained.joblib (13 features including HOD/LOD)
  - PWIN_THRESH = 0.50
...
MODEL_PATH   = "/Users/radhikaarora/Documents/Trading ML/ML V2/entry_model_pwin_13features_retrained.joblib"
PWIN_THRESH  = 0.50
ML_FEATURES  = [
...
class BNRLiveEngine13Retrained:
    ...
    def __init__(self, out_dir: str = "output/bnr_live_pwin13_retrained",
        ...
        if os.path.exists(MODEL_PATH):
            ...
            print(f"[BNR13R] ML model loaded ({len(self._features)} features, thresh={PWIN_THRESH})")
        else:
            self._model = None
            self._features = None
            print("[BNR13R] ML model not found — strong-candle fallback active")
    ...
    # ── ML scoring (13 features) ──────────────────────────────────────────────

    def _ml_score(self, retrace_ml: float, entry_price_est: float) -> Optional[float]:
        ...
        time_since_pivot = (
            (datetime.now(tz=ET) - self._pivot_time).total_seconds()
    ...
    def _close_trade(self, trade: LiveTrade, exit_ts: datetime,
        ...
        print(f"[BNR13R] EXIT {tag}  {exit_ts.strftime('%H:%M:%S')}  {exit_reason} | "
    ...
    def on_bar_30s(self, bar: dict):
        ...
        if self._candidate_active and not self._in_trade:
            if self._direction == 'long' and close < self._zone_low:
                print(f"[BNR13R] INVALIDATED long (30s close {close:.2f} < zone_low {self._zone_low:.2f})")
                self._reset_candidate()
                if close < self._zone_low:
                    self._direction = 'short'
                    self._candidate_active = True
                    print(f"[BNR13R] Breakout SHORT @ {close:.2f}  [30s]")
                return
            if self._direction == 'short' and close > self._zone_high:
                print(f"[BNR13R] INVALIDATED short (30s close {close:.2f} > zone_high {self._zone_high:.2f})")
                self._reset_candidate()
                if close > self._zone_high:
                    self._direction = 'long'
                    self._candidate_active = True
                    print(f"[BNR13R] Breakout LONG  @ {close:.2f}  [30s]")
                return
        ...
        if not self._candidate_active:
            if close > self._zone_high:
                self._direction = 'long'
                self._candidate_active = True
                print(f"[BNR13R] Breakout LONG  @ {close:.2f}  (zone_high={self._zone_high:.2f}) [30s]")
            elif close < self._zone_low:
                self._direction = 'short'
                self._candidate_active = True
                print(f"[BNR13R] Breakout SHORT @ {close:.2f}  (zone_low={self._zone_low:.2f}) [30s]")
    ...
    def on_bar_1m(self, bar: dict):
        ...
        if open_ts == session_open and self._zone_high is None:
            ...
            print(f"[BNR13R] Zone: H={self._zone_high:.2f}  L={self._zone_low:.2f}")
        ...
        if self._candidate_active and self._direction == 'long' and close < self._zone_low:
            ...
            self._target_price = None
            self._reset_retrace_accumulators()
            print(f"[BNR13R] Direction flip → SHORT @ {close:.2f}")
            ...
            self._target_price = None
            self._reset_retrace_accumulators()
            print(f"[BNR13R] Direction flip → LONG  @ {close:.2f}")

        if not self._candidate_active:
            if close > self._zone_high:
                self._direction = 'long'
                self._candidate_active = True
                print(f"[BNR13R] Breakout LONG  @ {close:.2f}  (zone_high={self._zone_high:.2f})")
            elif close < self._zone_low:
                self._direction = 'short'
                self._candidate_active = True
                print(f"[BNR13R] Breakout SHORT @ {close:.2f}  (zone_low={self._zone_low:.2f})")
            else:
                self._prev_1m_bar = b
                return
        ...
        if self._candidate_active and not self._reentry_seen:
            if self._direction == 'long':
                ...
                if close < b['open'] and low <= self._zone_high and high >= self._zone_low:
                    ...
                    self._pivot_time = ts
                    self._reset_retrace_accumulators()
                    print(f"[BNR13R] Retest/Pivot LONG  pivot={self._pivot:.2f}  flem={self._flem:.2f}")
            else:
                ...
                if close > b['open'] and low <= self._zone_high and high >= self._zone_low:
                    ...
                    self._pivot_time = ts
                    self._reset_retrace_accumulators()
                    print(f"[BNR13R] Retest/Pivot SHORT pivot={self._pivot:.2f}  flem={self._flem:.2f}")
        ...
        if self._reentry_seen and not self._entry_triggered:
            ...
            if self._direction == 'long' and low < self._zone_low:
                print(f"[BNR13R] INVALIDATED long (1m low {low:.2f} < zone_low {self._zone_low:.2f})")
                self._reset_candidate()
                if close < self._zone_low:
                    self._direction = 'short'
                    self._candidate_active = True
                    print(f"[BNR13R] Breakout SHORT @ {close:.2f}")
                self._prev_1m_bar = b
                return
            if self._direction == 'short' and high > self._zone_high:
                print(f"[BNR13R] INVALIDATED short (1m high {high:.2f} > zone_high {self._zone_high:.2f})")
                self._reset_candidate()
                if close > self._zone_high:
                    self._direction = 'long'
                    self._candidate_active = True
                    print(f"[BNR13R] Breakout LONG  @ {close:.2f}")
                self._prev_1m_bar = b
                return
            ...
            if self._model is not None and self._last_30s_close is not None:
                if self._flem is not None and self._pivot is not None and self._flem != self._pivot:
                    ...
                    score = self._ml_score(retrace_ml, self._last_30s_close)
            ...
            if (retrace is not None and
                ...
                if gate_ok:
                    ...
                    print(f"[BNR13R] ★ ENTRY {self._direction.upper()}  {ts.strftime('%H:%M:%S')} "
    ...
    def _handle_target_hit(self, trade: LiveTrade, ts: datetime, direction: str):
        if self._scale_out_active and self._scale_out_stage < len(self._scale_out_plan) - 1:
            ...
            self._trades[-1] = leg
            print(f"[BNR13R] SCALE OUT leg {self._scale_out_stage + 1} @ {leg_price:.2f} "
            ...
            self._scale_out_active = False
            self._scale_out_stage = 0
            print(f"[BNR13R] EXIT ✓ WIN  target @ {exit_price:.2f} | "
                  f"pnl={pnl_dollars:+.0f}$  daily={self._daily_pnl:+.0f}$")
    ...
    def save_trades_csv(self):
        if not self._trades:
            print("[BNR13R] No trades to save.")
            return
        day_str = str(self._current_date or datetime.now(tz=ET).date())
        path = os.path.join(self._out_dir, f"bnr13_trades_{day_str}.csv")
        fields = [
        ...
        print(f"[BNR13R] Saved {len(self._trades)} trade records → {path}")
